...
if (!function_exists('get_item_class')) {
	function get_item_class($menu_item)
	{
		...
		if (
			is_page_template('templates/template_about.php') ||
			is_page_template('templates/template_about_award.php') ||
			is_page_template('templates/template_about_brand.php') ||
		...
		if ($menu_item->children)
			$classes[] = 'has-children menu-item-has-children';
		if ($is_mega_menu)
			$classes[] = 'has-megamenu drop-mega mega-product';
							...
							<div class="row no-gutters">
								<div class="col w-full lg:w-8/12 relative">
									<ul class="nav-sub menu-lv-1">
										<?php foreach ($menu_level_1->children as $menu_level_2): ?>
											...
											?>
											<li class="nav-link">
												<div class="head"> <a href="<?= $menu_level_2->url ?>"
														title="<?= $menu_level_2->title ?>" <?= $menu_level_2->target ? ' target="' . htmlspecialchars($menu_level_2->target, ENT_QUOTES) . '"' : '' ?>><?= $menu_level_2->title ?></a><em class="fa-light fa-chevron-right"></em>
												</div>
												<ul class="nav-sub menu-lv-2">
													<?php foreach ($menu_level_2->children as $menu_level_3): ?>
														<li class="nav-link is-active">
															<div class="head"> <a href="<?= $menu_level_3->url ?>"
																	title="<?= $menu_level_3->title ?>" <?= $menu_level_3->target ? ' target="' . htmlspecialchars($menu_level_3->target, ENT_QUOTES) . '"' : '' ?>><?= $menu_level_3->title ?></a><em
																	class="fa-solid fa-chevron-down"></em></div>
															<ul class="nav-sub menu-lv-3">
																<?php foreach ($menu_level_3->children as $key => $menu_level_4):
																	$image = get_field('image', $menu_level_4->ID);
																	if (empty($image_first) && !empty($image)) {
																		$image_first = $image;
																		$title_first = $menu_level_4->title;
																		$url_first = $menu_level_4->url;
																	}
																	?>

																	<li class="nav-link" data-link="<?= $menu_level_4->url ?>"
																		data-title="<?= $menu_level_4->title ?>"
																		data-img="<?= !empty($image['url']) ? $image['url'] : '' ?>">
																		<a href="<?= $menu_level_4->url ?>" title="<?= $menu_level_4->title ?>"
																			<?= $menu_level_4->target ? ' target="' . htmlspecialchars($menu_level_4->target, ENT_QUOTES) . '"' : '' ?>><?= $menu_level_4->title ?></a>
																	</li>
																<?php endforeach; ?>
															</ul>
														</li>
								...
								<div class="col w-full lg:w-4/12">
									<div class="temp-product">
										<div class="img zoom-in overflow-hidden"><a href="<?= $url_first ?>"><img
													src="<?= $image_first['url'] ?>" alt="<?= $image_first['alt'] ?>"></a></div>
										<div class="txt-wrap h-fit">
											<h3 class="sub-title"><a
													class="underline center-item text-center temp-title font-bold text-primary-1 text-24"
													href="<?= $url_first ?>"><?= $title_first ?></a></h3>
										</div>
									</div>
								</div>
							</div>
