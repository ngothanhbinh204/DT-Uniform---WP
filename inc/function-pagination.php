...
function wp_bootstrap_pagination($args = array())
{
	...
	$defaults = array(
		...
		'before_output' => '<div class="pagination mt-base">
	...
	if ($firstpage && (1 != $page)) $echo .= '<li class="previous hidden"><a href="' . $firstpage . '"><i class="fa-solid fa-chevrons-left"></i></a></li>';
	...
	if (!empty($min) && !empty($max)) {
		for ($i = $min; $i <= $max; $i++) {
			if ($page == $i) {
				...
				$echo .= sprintf('<li><a href="%s">%01d</a></li>', esc_attr(get_pagenum_link($i)), $i);
			}
		}
	}
	$next = intval($page) + 1;
	...
	if ($lastpage) {
		$echo .= '<li class="next hidden"><a href="' . $lastpage . '"><i class="fa-solid fa-chevrons-right"></i></a></li>';
	}
