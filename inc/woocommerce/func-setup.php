...
function devqn_quantity_plus()
{
	...
	if (($product && !$product->is_sold_individually()) || (isset($GLOBALS['woocommerce']->cart) && !empty($GLOBALS['woocommerce']->cart->get_cart()))) {
		echo '<button type="button" class="plus quantity-input-button btn-ins btn-inc " >+</button>';
	}
}
...
function devqn_quantity_minus()
{
	...
	// Check if product exists and is not sold individually
	if (($product && !$product->is_sold_individually()) || (isset($GLOBALS['woocommerce']->cart) && !empty($GLOBALS['woocommerce']->cart->get_cart()))) {
		echo '<button type="button" class="minus quantity-input-button btn-ins btn-dec">-</button>';
	}
}
...
{
	echo '<div class="product-detail-quantity-flex">';
}
...
function woo_add_buy_now_button()
{
	...
	$type_product = get_field('type', $product_id);
	$button_contact_solution = get_field('button_contact_solution', 'option');
